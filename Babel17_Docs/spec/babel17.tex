...
\end{itemize}
There is no such language out there. Therefore I decided to create one. When Babel-17 will have reached v1.0 then the above goals will have been achieved. The version described in this document is Babel-17 v0.3.2, so there is still some distance to go. 
...
\babelsrc{$c\ $ $p$} & \patterndescr{here $c$ is a constructor; same as \babelsrc{($c\ $ \! $\ p$)}; instead of \babelsrc{$c\ $ $\_$} you can just write \babelsrc{$c$}}\\\hline
...
\babelsrc{($u\ $ \! $\ p$)} & \patterndescr{let $v$ be the value to be matched; then the match succeeds if the result $v$.\babelsrc{destruct\_}  $u$ matches $p$}\\\hline
\babelsrc{($u\ $ \!)} & \patterndescr{short for \babelsrc{($u\ $ \! \_)} }\\\hline
...
\end{babellisting}
The cases $op$ $\in \{==, <>\}$ do not propagate the \emph{Unrelated}-exception, and are defined like this:
\begin{babellisting}
match a $\sim$ b 
  case (u if u $op$ 0) => true
  case _ => false
...
typedef $t$ $p$
\end{babellisting}
Here $u_p$ is basically the pattern $p$ converted into a term. Not all patterns $p$ allow such a conversion; the shortcut notation is only available for patterns that do.
