...
{morekeywords={lazy, concurrent, force, match, case, exception, begin, end, random, choose,force,true, false, infinity, val, def, this, object,if,then,else,elseif,memoize,for,in,yield,while,do,with,nil, div, mod,try,catch,private,import,module,unittest,typedef,typeof,and, or, not},
sensitive=true, 
morecomment=[l]{//}, 
morecomment=[s]{/*}{*/}, 
morestring=[b]", 
...
\babelsrc{$(p : (e))$} & \patterndescr{matches anything that matches $p$ and has the type that $e$ evaluates to}
\end{tabular}
...
\label{tab:relops}
\end{table}
Note that $a$ and $b$ are unrelated iff $a \sim b$ is less than $-1$ or greater than $1$.
...
\section{Reals and Interval Arithmetic}
...
Modules allow to give Babel-17 code a static structure. Modules are introduced with the syntax
\begin{babellisting}
...
\end{babellisting}
where $p = p_1.p_2.\ldots.p_n$ is the \emph{module path}. A condition on module paths is that all elements of a module path must be different, i.e. $p_i \neq p_j$ for $i \neq j$. 

The statements $s_i$ that may appear in modules are basically those that may also appear in object expressions. Additionally, modules can contain \emph{type definitions}; this is the topic of the next section. The statements of a module are interpreted in a new empty namespace. With the exception of other modules, none of the values or definitions outside of a module are accessible inside a module.
...
A module is in one of the following three states: DOWN, LOADING or UP. There are four things that one might  suspect to change the state of the module: 
\begin{description}
\item[A] the module can be forced via \babelsrc{force\ $p$}.
\item[B] the module receives a message $m$ that corresponds to a definition inside of it,
\item[C] the module receives a message $m$ that corresponds to a submodule $p.m$,
\item[D] the module receives an invalid message $m$.
\end{description}
Of these four things, neither C nor D change the state of the module.

In the following we distinguish modules which need initialization from modules which don't. A module does not need initialization iff it consists only of \babelsrc{def}-, \babelsrc{typedef}-, \babelsrc{import}-statements.

If module $p$ needs initialization and is in state DOWN, then A and B cause $p$ to go to state LOADING; after $p$ has been loaded, that is all statements of the module have been executed, $p$ goes from state LOADING into state UP. If the module needs no initialization, the state goes directly from DOWN to UP.

If $p$ is in state LOADING, then A and B do not change the state of the module, but throw exceptions with parameter \babelsrc{BusyModule}. If $p$ is already in state UP, then A and B also do not change the state of the module. 

Note that the state of a module is not directly accessible and only indirectly observable via the \babelsrc{BusyModule}-exception.

Modules that need initialization can lead to nondeterministic behavior:
\begin{babellisting}
module needsInit
  val a = 10
  def x = a
end
(concurrent needsInit.x) * (concurrent needsInit.x)
\end{babellisting}
may yield the result $100$ or the exception \emph{BusyModule}. On the other hand,
\begin{babellisting}
module needsNoInit
  def x = 10
end
(concurrent needsNoInit.x) * (concurrent needsNoInit.x)
\end{babellisting}
will always yield $100$.
...
typedef $t$ $pat$ = $expr$
\end{babellisting}
where $t$ is an identifier, $pat_i$ a pattern and $expr_i$ an expression. Like a \babelsrc{def} statement, the above introduces a new function $t$. This new function $t$ has the property that all values $v$ it returns will have type $t$ and will consist of two components: an \emph{inner value}, and an \emph{outer value}. The inner value $i$ is just the argument that was passed to $t$. The outer value $o$ is the result of evaluating $expr$. 

The point of all this is that the value $v$ behaves just like its outer value $o$, except 
...
To avoid typing long module paths you can \emph{import} 


\section{
TODO: this in object checken and explainen}
...
\section{Standard Library (To Be Updated for v0.3)}
