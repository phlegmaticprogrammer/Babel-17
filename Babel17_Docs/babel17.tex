...
\babelsrc{$c$.isEmpty} & checks if $c$ is an empty collection \\
\babelsrc{$c$.empty} & an empty collection that has the same type as $c$ \\
\babelsrc{$c$.size} & the size of collection $c$ \\
\babelsrc{$c + x$} & adds $x$ as member to collection $c$ \\
\babelsrc{$c\ \text{++}\ d$} & adds all member of $d$ as members to $c$ \\
\babelsrc{$c$.head} & the first element of $c$ \\
\babelsrc{$c$.tail} & all elements of $c$ except the first one\\
\babelsrc{$c$.atIndex $\ i$} & the $i$-th element of $c$\\
\babelsrc{$c$.indexOf $\ x$} & the lowest $i$ such that \babelsrc{$c$.atIndex $\ i\ ==\ x$}  \\
\babelsrc{$c$.contains $\ x$} & checks if $c$ contains $x$ \\
\babelsrc{$c$.take $\ n$} & forms a collection out of the first $n$ elements of $c$ \\
\babelsrc{$c$.drop $\ n$} & forms a collection by dropping the first $n$ elements of $c$
\end{tabular}
...
\texttt{integer} & Boolean, String \\
\texttt{infinity} & String \\
\texttt{boolean} & Integer, String \\
\texttt{string} & Integer, Infinity, Boolean, List, Vector \\
\texttt{list} &  Vector, Set, Map, String \\
\texttt{vector} & List, Set, Map, String \\
\texttt{set} & List, Vector, Map \\
\texttt{map} & List, Vector, Set \\
\end{tabular}
...
\subsection{Boolean} Has no special messages.
...
Where will Babel-17 go from here? There are two dimensions along which Babel-17 has to grow in order to find broad adoption.

\subsection{Maturity and breadth of implementations}
...
\subsection{Development of the language} 
Right now Babel-17 is a purely functional programming language with no static typing and only a handful of built-in types. Modularization of code is possible via objects.

Encapsulation, more powerful types and some way of dealing with state, user interfaces, and communication with the outside world will be added to Babel-17. Programmers will be able to choose which of these future features they would like to employ. Those they don't care about they will be able to just ignore mostly and pretend that Babel-17 v21.0 is still Babel-17 v0.21.  
