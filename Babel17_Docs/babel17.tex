...
\babelsrc{\{->\}} & \patterndescr{matches the empty map (see section~\ref{sec:setsandmaps})}\\\hline
\babelsrc{\(for $\ p_1$,$\ \ldots\ $, $\ p_n\ $end\)} & \patterndescr{see section~\ref{sec:setsandmaps}}\\\hline
\babelsrc{\(for $\ p_1$,$\ \ldots\ $, $\ p_n$, $\ \delta\ $end\)} & \patterndescr{see section~\ref{sec:setsandmaps}}
\end{tabular}
...
$S / f$ & \tabparbox{set created by applying the function $f$ to the elements of $S$}
...
$M$ \verb!++! $N$ &  \tabparbox{map created from the map $M$ by adding the key/value pairs that are element of $N$}\\\hline
$M$ \verb!--! $S$ & \tabparbox{map created from $M$ by removing all keys that are elements or keys of $S$}\\\hline
...
and that memoization can only be requested for definitions that take an argument.
